...
try:
    ...
    print("Cannot install dependencies: {}".format(e))
...
def parse_args():
    ...
    parser.add_argument('--model_dir', type=str, default=os.environ['SM_MODEL_DIR'], help='Output path for the model')
    parser.add_argument('--train', type=str, default=os.environ['SM_CHANNEL_TRAIN'], help='Path for the training images')
    
    args = parser.parse_args()
    return args

class VOCLike(gdata.VOCDetection):
    CLASSES = ("shell","elbow","penne","tortellini","farfalle")
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        dataset_root_folder=args.train
        arr = os.listdir(dataset_root_folder)
        print(arr)
        train_dataset = VOCLike(root=dataset_root_folder, splits=[(2019, 'trainval')])
        val_dataset = VOCLike(root=dataset_root_folder, splits=[(2019, 'test')])
